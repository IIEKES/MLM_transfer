...
def load_params_val(path):
    ...
    with open(path, 'rb') as f:  # open file with write-mode
        while f:
            try:
                param_dict = cPickle.load(f, encoding='latin1')
            except:
                break
    return param_dict
...
def get_params_file_name(conf_dict) :
    ...
    """
    conf_str = ','.join(['{0}:{1}'.format(key, value) for key, value in conf_dict.items()])
    return conf_dict['algo_name'] + '_' + hashlib.md5(conf_str.encode(conf_dict['charset'])).hexdigest()
...
def save_confs_val(conf_dict, path):
    ...
    with open(path, 'wb') as fw:
        for conf in conf_dict:
            fw.writelines(bytes((str(conf) + ' = ' + str(conf_dict[conf]) + '\n'),charset))